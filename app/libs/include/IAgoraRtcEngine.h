...
public:
  ...
  * @param [in] info
    ...
    */
    ...
    /**
    * gameStart the echo testing, if every thing goes well you can hear your echo from the server
    * @return return 0 if success or an error code
    */
    ...
    */
    ...
    /**
    * gameStart the local video previewing
    * @return return 0 if success or an error code
    */
  ...
  /**
    ...
    */
    ...
    int setRemoteVideoStreamType(uid_t uid, REMOTE_VIDEO_STREAM_TYPE streamType) {
        return setObject("rtc.video.set_remote_video_stream", "{\"uid\":%u,\"stream\":%e}", uid, streamType);
    }
    ...
    */
    int enableAudioVolumeIndication(int interval, int smooth) { // in ms: <= 0: disable, > 0: enable, interval in ms
        if (interval < 0)
            interval = 0;
        return setObject("che.audio.volume_indication", "{\"interval\":%e,\"smooth\":%e}", interval, smooth);
    }

    /**
    * gameStart recording the audio stream
    * @param [in] filePath
	...
	*/
	int startAudioMixing(const char* filePath, bool loopback, bool replace, int cycle) {
		...
		if (!m_parameter->convertPath(filePath, path))
			...
			return -ERR_INVALID_ARGUMENT;
#endif
		return setObject("che.audio.start_file_as_playout", "{\"filePath\":\"%s\",\"loopback\":%s,\"replace\":%s,\"cycle\":%e}",
					filePath,
					loopback?"true":"false",
					replace?"true":"false",
					cycle);
    ...
    */
    int setRemoteRenderMode(uid_t uid, RENDER_MODE_TYPE renderMode) {
        return setObject("che.video.render_mode", "{\"uid\":%u,\"mode\":%e}", uid, renderMode);
    }
    ...
    int setRecordingAudioFrameParameters(int sampleRate, int channel, RAW_AUDIO_FRAME_OP_MODE_TYPE mode, int samplesPerCall) {
        return setObject("che.audio.set_capture_raw_audio_format", "{\"sampleRate\":%e,\"channelCnt\":%e,\"mode\":%e,\"samplesPerCall\":%e}", sampleRate, channel, mode, samplesPerCall);
    }
    int setPlaybackAudioFrameParameters(int sampleRate, int channel, RAW_AUDIO_FRAME_OP_MODE_TYPE mode, int samplesPerCall) {
        return setObject("che.audio.set_render_raw_audio_format", "{\"sampleRate\":%e,\"channelCnt\":%e,\"mode\":%e,\"samplesPerCall\":%e}", sampleRate, channel, mode, samplesPerCall);
    }
